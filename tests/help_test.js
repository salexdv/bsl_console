...
function testSearch() {
    ...
    const prefixItems = [
        { id: 'first', title: 'Ёлка' },
        { id: 'second', title: 'елка большая' },
        { id: 'third', title: 'Работа-со строкой' },
        { id: 'fourth', title: 'Число' },
        { id: 'duplicate-1', title: 'СТРОКА' },
        { id: 'duplicate-2', title: 'строка' }
    ];
    const prefixIndex = search.preparePrefixIndex(prefixItems);
    result = search.prefixSearch(prefixIndex, 'ЕЛ', 1000);
    assert.deepEqual(result.items.map(item => item.id), ['first', 'second'], 'е/ё и регистр эквивалентны');
    result = search.prefixSearch(prefixIndex, 'работа со', 1000);
    assert.deepEqual(result.items.map(item => item.id), ['third'], 'знаки препинания нормализуются как пробелы');
    result = search.prefixSearch(prefixIndex, 'строка', 1);
    assert.equal(result.total, 2, 'total не ограничивается размером выдачи');
    assert.deepEqual(result.items.map(item => item.id), ['duplicate-1'], 'дубликаты сохраняют стабильный порядок');
    result = search.prefixSearch(prefixIndex, '', 3);
    assert.equal(result.total, prefixItems.length);
    assert.equal(result.items.length, 3, 'пустой запрос соблюдает limit');
    ...
    const merged = search.mergePrefixIndexes(left, right);
    assert.deepEqual(search.prefixSearch(merged, 'строка', 10).items.map(item => item.id), ['l2', 'r2'],
        'слияние сохраняет порядок одинаковых ключей');

    let titleConversions = 0;
    const largePrefixItems = Array.from({ length: 20000 }, (_, i) => ({
        id: String(i),
        title: { toString() { titleConversions++; return (i % 2 ? 'Строка ' : 'Число ') + i; } }
    }));
    const largePrefixIndex = search.preparePrefixIndex(largePrefixItems);
    assert.equal(titleConversions, largePrefixItems.length, 'каждый заголовок нормализуется один раз при подготовке');
    search.prefixSearch(largePrefixIndex, 'с', 1000);
    search.prefixSearch(largePrefixIndex, 'стр', 1000);
    search.prefixSearch(largePrefixIndex, 'число 199', 1000);
    assert.equal(titleConversions, largePrefixItems.length, 'поиск не нормализует заголовки повторно');
...
async function testBase64Transfer() {
    ...
    const secondMegabyte = Buffer.alloc(1024 * 1024, 0x5a);
    ...
    RegExp.prototype.test = function () {
        regexpCalls++;
        return regexpTest.apply(this, arguments);
    };
    const started = process.hrtime.bigint();
    try {
        manager.push(megabyte.toString('base64'));
        manager.push(secondMegabyte.toString('base64'));
    ...
    assert.equal(large.length, megabyte.length + secondMegabyte.length + last.length);
    assert.deepEqual(large.subarray(0, megabyte.length), megabyte);
    assert.deepEqual(large.subarray(megabyte.length, megabyte.length + secondMegabyte.length), secondMegabyte);
    assert.deepEqual(large.subarray(megabyte.length + secondMegabyte.length), last);
    assert(regexpCalls <= 6, 'на большой порции нет regex-проверки каждой четвёрки');
    console.log(`[help-test] Base64: 2 МБ за ${elapsedMs.toFixed(1)} мс, RegExp.test: ${regexpCalls}`);
...
async function testService() {
    ...
    const internalState = service.getState();
    const publicState = service.getPublicState();
    assert.notStrictEqual(publicState.packages, internalState.packages);
    assert.notStrictEqual(publicState.packages.context, internalState.packages.context);
    assert.notStrictEqual(publicState.packages.context.navigation, internalState.packages.context.navigation);
    assert.notStrictEqual(publicState.packages.context.index, internalState.packages.context.index);
    assert.notStrictEqual(publicState.packages.context.stats, internalState.packages.context.stats);
    assert.notStrictEqual(publicState.kinds, internalState.kinds);
    publicState.packages.context.pages = -1;
    publicState.packages.context.navigation[0].title = 'испорченная навигация';
    publicState.packages.context.navigation[0].children.push({ title: 'лишний узел' });
    publicState.packages.context.index[0].key = 'испорченный индекс';
    publicState.packages.context.index[0].item.title = 'испорченная статья';
    publicState.packages.context.stats.strategy = 'испорченная статистика';
    publicState.kinds[0] = 'dcs';
    publicState.packages.context = null;
    const freshPublicState = service.getPublicState();
    assert.equal(internalState.packages.context.pages, 20);
    assert.equal(internalState.packages.context.navigation[0].title, 'context');
    assert.equal(internalState.packages.context.navigation[0].children.length, 0);
    assert.equal(internalState.packages.context.index[0].key, 'context');
    assert.equal(internalState.packages.context.index[0].item.title, 'context');
    assert.equal(internalState.packages.context.stats.strategy, undefined);
    assert.equal(internalState.kinds[0], 'context');
    assert.equal(freshPublicState.packages.context.pages, 20);
    assert.equal(freshPublicState.packages.context.navigation[0].title, 'context');
    assert.equal(freshPublicState.packages.context.navigation[0].children.length, 0);
    assert.equal(freshPublicState.packages.context.index[0].key, 'context');
    assert.equal(freshPublicState.packages.context.index[0].item.title, 'context');
    assert.equal(freshPublicState.packages.context.stats.strategy, undefined);
    assert.equal(freshPublicState.kinds[0], 'context');
    ...
    assert.equal(service.prefix('LANG', 1000).total, 1, 'service использует объединённый подготовленный индекс');
