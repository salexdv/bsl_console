...
* Сборка **single-file**: `npm run build:single` собирает `./dist/index.html` как единый
  самодостаточный HTML без внешних файлов (все ресурсы инлайнятся) — для загрузки в «Поле HTML
  документа» 1С 8.3.27+ текстом, в обход предупреждения безопасности об открытии локального файла.
  Проверка отсутствия внешних ссылок — `npm run check:single`. Обычная сборка не изменена.
