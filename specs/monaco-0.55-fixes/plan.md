...
### Известное ограничение: `build:pack` на Linux

- В 1С под Linux сборка `npm run build:single` передаёт события JavaScript → 1С штатно.
- В полной сборке `npm run build:pack` JavaScript-обработчик `document.onclick` выполняется, но
  событие `ПолеHTMLПриНажатии` в 1С не генерируется. Причина не установлена.
- Изолированные стенды с динамическим `appendChild` и с цепочкой
  gzip/base64 → `pako.ungzip` → `appendChild` в том же окружении работают. Поэтому связывать дефект
  только с `appendChild`, `pako` или сжатием моста событий оснований нет.
- До выяснения причины `build:pack` на Linux считается экспериментальной; для 1С нужно использовать
