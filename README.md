...
Для работы внутри 1С требуется версия платформы не ниже **8.3.24** (обновлённый WebKit)
...
На основе [Monaco editor](https://github.com/Microsoft/monaco-editor), версии [0.55.1](https://github.com/microsoft/monaco-editor/releases/tag/v0.55.1)
...
* Определение типа переменной по коду: конструктор `Новый`, присваивание, результат метода или функции
* Подсказка имён ключей структуры и колонок таблицы значений, заданных в коде
* [Объявление типа комментарием](docs/type_hints.md) в синтаксисе строгой типизации ЕДТ
...
##### Сборка single-file (один самодостаточный HTML для поля 1С 8.3.27+)

Собирает `./dist/index.html` как **единый самодостаточный файл без внешних ресурсов**: JS, CSS,
шрифты, картинки, иконки дерева и воркер Monaco инлайнятся в сам HTML (`data:`/`blob:`). Это нужно,
чтобы загружать редактор в «Поле HTML документа» 1С **как текст** и обойти предупреждение безопасности
платформы **8.3.27+** «ПолеHTMLДокумента пытается открыть локальный файл» (оно возникает на любой
ресурс, читаемый с диска).

```js
npm run build:single
```

После сборки в `./dist/` остаётся **ровно один файл** `index.html`. Проверить отсутствие внешних
ссылок можно отдельно:

```js
npm run check:single
```

Обычная сборка (`npm run build`) при этом не меняется.

##### Сборка сжатого single-file (один самодостаточный HTML для поля 1С 8.3.27+ меньшего размера)
...
4. На платформе **8.3.27+** (с включённой «Защитой от опасных действий») собрать single-file
   (`npm run build:single`) и загрузить содержимое `./dist/index.html` в «Поле HTML документа»
   **как текст** — так платформа не выдаёт предупреждение об открытии локального файла.
...
| [`setQueryDescription`](docs/set_query_description.md) | Показ примечаний подзапросов у правого края строк в режимах запроса/СКД |
...
* В веб-клиенте недоступно любое взаимодействие редактора и 1С. Можно попробовать только набор кода, отображение кода, вывод режима сравнения.
