...
> Сборка `npm run build:pack` в этом окружении работает некорректно: JavaScript-обработчик клика
> выполняется, но событие `ПолеHTMLПриНажатии` в 1С не генерируется. Причина пока не установлена,
> поэтому `build:pack` на Linux считается экспериментальной и не поддерживается.
