...
class SearchHistoryController {
  ...
  attachEditor(editor) {
    ...
    this.syncControllerHistory(controller);
    this.ensureWidgetAdapter(editor);
  }
  ...
  syncMonacoHistory() {
    const synchronizedServices = new Set();
    this.editors.forEach(binding => {
      const service = binding.controller && binding.controller._findWidgetSearchHistory;
      if (service && !synchronizedServices.has(service) && typeof service.replace == 'function') {
        synchronizedServices.add(service);
        service.replace(this.history.slice().reverse());
      }
    });
  }

  syncControllerHistory(controller) {
    const service = controller && controller._findWidgetSearchHistory;
    if (service && typeof service.replace == 'function')
      service.replace(this.history.slice().reverse());
  }
  ...
  ensureWidgetAdapter(editor) {
    ...
    if (!findInput || !inputBox || !domNode || typeof inputBox.onDidBlur != 'function')
      return;
    ...
    adapter.disposables.push(inputBox.onDidBlur(() => this.add(inputBox.value)));
    adapter.disposables.push(this.listen(inputBox.inputElement, 'keydown', event => {
...
export { SEARCH_HISTORY_LIMIT, normalizeSearchHistory, addSearchHistoryValue, SearchHistoryController };
