...
// в DecorationsOverlay Monaco порядок в строке определяется сортировкой по zIndex,
// иначе сплошной фон подсветки перекрывает текст подписи.
const QUERY_DESCRIPTION_Z_INDEX = 100;

// Экранирует текст для CSS-строки в динамическом правиле content (по образцу реализации
// inlay-hints на Monaco 0.20; в этой ветке инлей-хинты нативные, хелпер живёт здесь).
...
export function createQueryDescriptionsController(codeEditor) {
  ...
  // правого края контента (Monaco 0.55 не вычитает его из contentWidth), а включённый
  ...
  function render() {
    ...
    for (let i = 0; i < anchors.length; i++) {
      ...
      rules.push('.' + QUERY_DESCRIPTION_MARKER + '.' + className + '::after { content: "' + escapeCssString(anchors[i].text) + '"; right: ' + right + '; }');
