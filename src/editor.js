...
define(['bslGlobals', 'bslMetadata', 'snippets', 'bsl_language', 'vs/editor/editor.main', 'actions', 'bslQuery', 'bslDCS', 'colors'], function () {
  ...
  setTheme = function (theme) {
    ...
    if (editor && editor.inlineSuggestController)
      editor.inlineSuggestController.layout();
  ...
  showCustomInlineSuggestions = function(suggestions) {
    ...
    try {
      ...
      for (const [key, value] of Object.entries(suggestObj)) {
        ...
        customInlineSuggestions.push(item);
  ...
  showInlineSuggestion = function(lineNumber, column, text) {

    if (!editor || !editor.inlineSuggestController)
      return false;

    return editor.inlineSuggestController.showText(lineNumber, column, text);

  }

  hideInlineSuggestions = function () {

    if (editor && editor.inlineSuggestController)
      editor.inlineSuggestController.hide();

  }

  triggerInlineSuggestions = function () {

    if (editor && editor.inlineSuggestController)
      editor.inlineSuggestController.trigger(true);

  }

  isInlineSuggestionsVisible = function () {

    return editor && editor.inlineSuggestController ? editor.inlineSuggestController.isVisible() : false;

  }

  enableInlineSuggestions = function (enabled) {

    inlineSuggestEnabled = enabled;
  ...
  activateInlineCompletionsApi = function() {
    ...
    if (!monaco.languages.registerInlineCompletionsProvider) {
      monaco.languages.registerInlineCompletionsProvider = function (languageSelector, provider) {
        ...
        inlineCompletionProviders.push(record);

        return {
          dispose: function () {
            inlineCompletionProviders = inlineCompletionProviders.filter(item => item !== record);
          }
        };
  ...
  function initEditorEventListenersAndProperies() {
    ...
    editor.checkBookmarks = true;
    editor.diff_decorations = [];
    editor.ifDecorations = [];
    editor.inlineSuggestController = createInlineSuggestController(editor);
    ...
    editor.onDidChangeModelContent(e => {
      ...
      editor.inlineSuggestController.trigger(false, getInlineTriggerCharacter(e));
    ...
    editor.onDidChangeCursorSelection(e => {
      ...
      if (e.selection.isEmpty())
        editor.inlineSuggestController.trigger(false);
      else
        editor.inlineSuggestController.hide();
  ...
  function createInlineCompletionFromText(lineNumber, column, text) {
    ...
    if (!editor || !editor.getModel() || typeof text != 'string')
      return null;
  ...
  function createInlineSuggestController(codeEditor) {
    ...
    return {
      ...
      showText: function (lineNumber, column, text) {
        ...
        if (!inlineSuggestEnabled || readOnlyMode || codeEditor.navi || !codeEditor.hasModel())
          return false;
  ...
  function handleEventSuggestSelection(suggestItem) {
    ...
    hideSuggestionsList();
    sendEvent(completion.eventName, eventParams);
    setTimeout(() => {
      editor.focus();
    }, 0);
    return true;
  ...
  disposeEditor = function() {
    ...
    if (editor) {
      ...
      if (editor.inlineSuggestController)
        editor.inlineSuggestController.dispose();

      if (editor.diffRevertButtons)
          editor.diffRevertButtons.dispose();
  ...
  function applyDiffAllowRevertBackOptions(diff_editor) {
    ...
    original_editor.updateOptions({
      readOnly: readOnlyMode || !original_editable,
  ...
  function revertDiffChange(diff_editor, change_index) {
    ...
    if (getOption('generateModificationEvent'))
      sendEvent('EVENT_CONTENT_CHANGED', '');
    const disposable = diff_editor.onDidUpdateDiff(() => {
        disposable.dispose();
        sendEvent('EVENT_COMPARE_COMPLETE', {});
    });
  }
  ...
  function editorOnKeyDown(e) {
    ...
    if (e.keyCode == 16 && editor.getPosition().lineNumber == 1)
      ...
      // Tab
      if (editor.inlineSuggestController && editor.inlineSuggestController.accept()) {
        e.preventDefault();
        e.stopPropagation();
        return;
      }
