...
(async function () {
  ...
  try {
    ...
    await page.evaluate(function () {
      ...
      window.__indexMetaObserver = new MutationObserver(function (records) {
        window.__indexMetaMutations += records.length;
      });
      window.__indexMetaObserver.observe(document.querySelector('.bsl-help-panel[data-tab=index] .bsl-help-meta'), {
    ...
    const multipleLookup = await page.evaluate(function () {
      ...
      return {
        ...
        events: window.__capturedHelpEvents.slice(),
        indexRenders: window.__indexMetaMutations
      };
    });
    ...
    await page.waitForFunction('document.querySelector(".bsl-help-panel[data-tab=index] .bsl-help-input").value == "СТР"'
      + ' && document.querySelectorAll(".bsl-help-panel[data-tab=index] .bsl-help-list-row").length > 0');
    const indexTyping = await page.evaluate(function () {
      ...
      return {
        context: { text: context && context.textContent, title: row.title, color: context && getComputedStyle(context).color },
        renders: window.__indexMetaMutations
      };
    });
    if (indexTyping.renders != 1)
      errors.push('index input was not coalesced: ' + JSON.stringify(indexTyping));
    ...
    await page.waitForFunction('document.querySelector(".bsl-help-panel[data-tab=index] .bsl-help-list-title").textContent == "Число"');
    await page.$eval('.bsl-help-panel[data-tab="index"] .bsl-help-input', function (input) {
      input.value = 'СТР';
      input.dispatchEvent(new Event('input', { bubbles: true }));
    });
    ...
    async function shortcutFor(mode, diffSide) {
      ...
      if (mode == 'diff') {
        ...
        // Устанавливаем тестовую позицию после завершения этого цикла, как при реальном вводе.
        await page.evaluate(function (requestedSide) {
          const target = requestedSide == 'original'
            ? window.editor.getOriginalEditor()
            : window.editor.getModifiedEditor();
          target.setPosition({ lineNumber: 1, column: 3 });
          target.focus();
        }, diffSide);
      }
