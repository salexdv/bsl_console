...
* `originalEditable` - *boolean*, включает/отключает редактирование оригинального кода в режиме сравнения, включает/отключает возможность поблочного отката изменений