...
* Поиск слова под курсором в справке текущего режима `CTRL+F1`: `shcntx` + `shlang` для `bsl`/`xml`, `shquery` для `bsl_query`, `dcsui` для `dcs_query`. Готовая справка открывает первый результат автоматически. Без готового профильного пакета панель не открывается, но опция [`generateGetHelpEvent`](set_option.md) позволяет отправить [`EVENT_ON_GET_HELP`](get_help_event.md); обычный `F1` остаётся за Monaco/платформой.
