...
* Поиск слова под курсором в индексе синтакс-помощника 1С `CTRL+F1`. Готовая справка открывает первый результат автоматически. Без готовой справки панель не открывается, но опция [`generateGetHelpEvent`](set_option.md) позволяет отправить [`EVENT_ON_GET_HELP`](get_help_event.md); обычный `F1` остаётся за Monaco/платформой.
